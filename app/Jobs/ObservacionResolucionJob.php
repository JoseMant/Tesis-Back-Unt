...
use App\Mail\ObservacionResolucionMail;

class ObservacionResolucionJob implements ShouldQueue
{
    ...
    public function handle()
    {
        if ($this->usuario->correo2) {
            ...
            ->send(new \App\Mail\ObservacionResolucionMail($this->usuario,$this->tramite,$this->tipo_tramite,$this->tipo_tramite_unidad));
        }else{
            Mail::to($this->usuario->correo)
            ->send(new \App\Mail\ObservacionResolucionMail($this->usuario,$this->tramite,$this->tipo_tramite,$this->tipo_tramite_unidad));
        }
