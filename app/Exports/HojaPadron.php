    ...
    */
    ...
    public function collection()
    {
        ...
        if ($this->tipo_emision=='O') {
            ...
            'usuario.tipo_documento',
            'usuario.nro_documento',
            DB::raw('CONCAT("") AS MATRI_FEC'),
            DB::raw('CONCAT("") AS EGRES_FEC'),
            DB::raw('substr(tipo_tramite_unidad.diploma_obtenido,1, 1) AS ABRE_GYT'),
            DB::raw("(case 
                        when tramite.idUnidad = 1 then programa.nombre
                        when tramite.idUnidad = 4 then programa.denominacion
            ...
            DB::raw('CONCAT("") AS PROC_INST_ORIG'),
            DB::raw('CONCAT("") AS PROC_TITULO_PED'),
            DB::raw('CONCAT("") AS PROG_ESTU'),
            DB::raw('CONCAT("") AS NUM_CRED'),
            'modalidad_carpeta.nombre_padron AS MOD_OBT',
            DB::raw('CONCAT("") AS MOD_EST'),
            DB::raw('CONCAT("") AS REG_METADATO'),
            DB::raw('CONCAT("") AS TRAB_INV'),
            DB::raw('CONCAT("") AS REQ_IDM'),
            DB::raw('CONCAT("") AS PROG_ACREDIT'),
            DB::raw('CONCAT("") AS FEC_INICIO_ACREDIT'),
            DB::raw('CONCAT("") AS FECHA_FIN_ACREDIT'),
            DB::raw('CONCAT("") AS FEC_INI_TRA_TIT'),
            DB::raw('CONCAT("") AS TRAB_INVEST_ORIGINAL'),
            DB::raw('CONCAT("") AS MEC_UTI'),
            DB::raw('CONCAT("") AS DEP_VER_ORIG'),
            DB::raw('CONCAT("") as PROC_REV_PAIS'),
            DB::raw('CONCAT("") as PROC_REV_UNIV'),
            DB::raw('CONCAT("") as PROC_REV_GRADO'),
            DB::raw('CONCAT("") as CRIT_REV'),
            DB::raw('CONCAT("") as RESO_NUM'),
            DB::raw('CONCAT("") as RESO_FEC'),
            DB::raw('resolucion.nro_resolucion as RESO_NUM_DUP_NUE'),//----
            DB::raw('resolucion.fecha as RESO_FEC_DUP_NUE'),
            'cronograma_carpeta.fecha_colacion AS DIPL_FEC_ORG', //FECHA DE COLACIÓN
            DB::raw('tramite_detalle.fecha_emision_duplicado as DIPL_FECHA_DUP_NUEVO'),
            'tramite_detalle.codigo_diploma AS DIPL_NUM',
            DB::raw('CONCAT("DUPLICADO") as DIPL_TIP_EMI'),// HACERLO DINÁMICO CON UN NUEVO CAMPO
            'tramite_detalle.nro_libro AS REG_LIBRO',
            'tramite_detalle.folio AS REG_FOLIO',
            'tramite_detalle.nro_registro AS REG_REGISTRO',
            DB::raw("(case 
