...
class ReporteGradoObservadosExport implements WithTitle,ShouldAutoSize, WithEvents,FromArray,WithColumnFormatting
{
    public $arrays;
    public $datos;
    /**
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle('B1:F1')->getFont()->setBold(true)->setSize(14);
                $event->sheet->getDelegate()->getStyle('B1:F1')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
                $event->sheet->getDelegate()->getStyle('B1:F1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $event->sheet->getDelegate()->getStyle('B1:F1')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('0FD9F1');
                foreach ($this->datos as $key => $dato) {
                    // Añadiendo estilos al subtitulo de cada programa 
                    $rango="B".($dato[0]+1).":"."F".($dato[0]+1);
                    ...
                    $rango="B".($dato[0]+3).":"."F".($dato[0]+2+$dato[1]);
                    $event->sheet->getDelegate()->getStyle($rango)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
                    $event->sheet->getDelegate()->getStyle($rango)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                    // Creando los bordes de la tabla
                    $rango="B".($dato[0]+2).":"."F".($dato[0]+2+$dato[1]);
